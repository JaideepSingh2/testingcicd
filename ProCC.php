...
$dbname = "agms";
        ...
        $result = $conn->query("CALL getcustomersbycity('$city')");
        if ($result->num_rows > 0) {
            echo "<table><tr><th>CUSTID</th><th>FNAME1</th><th>LNAME1</th><th>STREET_ADDRESS</th><th>CITY</th><th>DOB</th><th>GID</th></tr>";
            // output data of each row
            while($row = $result->fetch_assoc()) {
                echo "<tr><td>".$row["CUSTID"]."</td><td>".$row["FNAME1"]."</td><td>".$row["LNAME1"]."</td><td>".$row["STREET_ADDRESS"]."</td><td>".$row["CITY"]."</td><td>".$row["DOB"]."</td><td>".$row["GID"]."</td></tr>";
            }
            echo "</table>";
        } else {
            echo "<table><tr><td colspan='7'>No results found</td></tr></table>";
        }
        $conn->close();
    }
    ?>
