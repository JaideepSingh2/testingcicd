...
<html>
<head>
    <title>Insertion in Artist</title>
...
$dbname = "agms";
    ...
    $result = $conn->query("CALL GetArtworksByArtist($artistid)");
    if ($result->num_rows > 0) {
        echo "<table><tr><th>ARTID</th><th>TITLE</th><th>YEAR</th><th>PRICE</th><th>CUSTID</th><th>GID</th><th>ARTID</th><th>ARTISTID</th><th>EID</th></tr>";
        // output data of each row
        while($row = $result->fetch_assoc()) {
            echo "<tr><td>".$row["ARTID"]."</td><td>".$row["TITLE"]."</td><td>".$row["YEAR"]."</td><td>".$row["PRICE"]."</td><td>".$row["CUSTID"]."</td><td>".$row["GID"]."</td><td>".$row["ARTID"]."</td><td>".$row["ARTISTID"]."</td><td>".$row["EID"]."</td></tr>";
        }
        echo "</table>";
    } else {
